...
from models.CamoEval_codsod_mask_fixation import UEDGNet as Network
...
'''
def save_fixation_gray(fix_pred, name, save_dir):
    """
    Save fixation map as grayscale image
    Args:
        fix_pred: numpy array of shape (H, W) with values in [0, 1]
        name: base name for the image
        save_dir: directory to save the image
    """
    # Normalize to 0-255 if not already
    if fix_pred.max() <= 1.0:
        fix_pred = (fix_pred * 255).astype(np.uint8)
    else:
        fix_pred = fix_pred.astype(np.uint8)

    os.makedirs(save_dir, exist_ok=True)
    Image.fromarray(fix_pred).save(os.path.join(save_dir, f"{name}.png"))
'''

def save_fixation_gray(fix_pred, mask, name, save_dir):
    """
    Save fixation map as grayscale image with only mask region showing
    Args:
        fix_pred: numpy array of shape (H, W) with values in [0, 1] (fixation)
    ...
    # Apply mask to zero out non-mask regions
    masked_fixation = fix_pred * mask

    # Normalize only within the mask region to preserve contrast
    if masked_fixation[mask > 0].size > 0:  # Check if there are non-zero values
    ...
    try:
        for i, (images, mask, gts, seg_label) in enumerate(train_loader, start=1):
            ...
            with torch.cuda.amp.autocast():
                sem, preds, _ = model(images, mask)

                #loss_sem = weighted_segmentation_loss(sem,seg_label)
                loss_cls = classification_loss(preds, gts-1)
                loss = loss_cls
...
def val(test_loader, model, epoch, save_path, writer):
    ...
    with torch.no_grad():
        ...
        for i, (image, mask, gt, seg_label, names) in enumerate(test_loader):
            ...
            sem = F.softmax(sem, dim=1)  # (B, 10, H, W)
            sem = sem.argmax(dim=1)+1
            fg_mask = (mask.squeeze(1) > 0).long()  # [B, H, W]
            sem = sem*fg_mask.float()

            fix = fix.squeeze(1)  # Remove channel dimension [B, H, W]

            for b in range(sem.shape[0]):
                sem_pred = sem[b].cpu().numpy()  # (H, W)
                sem_gt = seg_label[b].cpu().numpy()
                fix_pred = fix[b].cpu().numpy()  # (H, W)
                mask_np = mask[b].squeeze().cpu().numpy()  # Get corresponding mask [H, W]
                ...
                save_fixation_gray(fix_pred, mask_np,img_name, save_dir_fix)  # Save fixation map
        ...
        if epoch == 1:
            ...
            print(f'[Cur Epoch: {epoch}] Metrics (rMAE={MAE})')
            logging.info(f'[Cur Epoch: {epoch}] Metrics (rMAE={MAE})')
        else:
            ...
            print(f'[Current Epoch {epoch}] rMAE={MAE}\n'
                  f'[Best Epoch {best_epoch}] rMAE={MAE}')
            logging.info(f'[Current Epoch {epoch}] rMAE={MAE}\n'
                         f'[Best Epoch {best_epoch}] rMAE={MAE}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default='./log/MyTrain_CamoFixation_codsod_mask_fixation_new/',
