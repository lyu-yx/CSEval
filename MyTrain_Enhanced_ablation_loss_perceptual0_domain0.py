...
from models.CamoEval_enhanced_dofixre8 import EnhancedDegreeNet as Network
...
def train(train_loader, model, optimizer, epoch, save_path, writer, criterion):
    ...
    try:
        ...
        # Log harmonization weights for monitoring
        if hasattr(model, 'get_harmonization_weights'):
            harmony_weights = model.get_harmonization_weights()
            for name, val in harmony_weights.items():
                writer.add_scalar(f'Harmonizer/{name}', val, global_step=epoch)
                logging.info(f'[Train] Epoch [{epoch:03d}] Harmonizer {name}: {val:.4f}')
...
def val(test_loader, model, epoch, save_path, writer):
    """Enhanced validation with semantic part visualization and harmonizer logging"""
    ...
    with torch.no_grad():
        ...
        if hasattr(model, 'get_harmonization_weights'):
            harmony_weights = model.get_harmonization_weights()
            for name, val in harmony_weights.items():
                writer.add_scalar(f'Harmonizer/{name}_val', val, global_step=epoch)
                logging.info(f'[Val] Epoch [{epoch:03d}] Harmonizer {name}: {val:.4f}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default='./log/MyTrain_Enhanced_ablation_loss_perceptual0_domain0/',
                        help='the path to save model and log')

    # Enhanced loss parameters (enhanced features are default)
    parser.add_argument('--alpha_perceptual', type=float, default=0, help='perceptual loss weight')
    parser.add_argument('--alpha_domain', type=float, default=0, help='domain loss weight')  # ori:0.1
    # Schedule parameters for domain adapter
    parser.add_argument('--freeze_domain_epoch', type=int, default=5,
                        help='number of epochs to freeze DomainAdapter and set domain loss weight to 0')
    parser.add_argument('--warmup_domain_epoch', type=int, default=10,
                        help='epochs to gradually ramp domain loss weight from 0 to alpha_domain after freezing period')
    ...
    for epoch in range(1, opt.epoch):
        # ------------------------
        # Dynamic domain-adapter schedule
        # ------------------------
        if epoch <= opt.freeze_domain_epoch:
            # Freeze DomainAdapter params and disable domain loss
            for p in model.domain_adapter.parameters():
                p.requires_grad = False
            criterion.alpha_domain = 0.0
        else:
            # Unfreeze params
            for p in model.domain_adapter.parameters():
                p.requires_grad = True

            # Linear warmup of alpha_domain
            warm_epoch = epoch - opt.freeze_domain_epoch
            if warm_epoch < opt.warmup_domain_epoch:
                ramp_ratio = warm_epoch / opt.warmup_domain_epoch
                criterion.alpha_domain = opt.alpha_domain * ramp_ratio
            else:
                criterion.alpha_domain = opt.alpha_domain

        # Log current alpha_domain for monitoring
        writer.add_scalar('alpha_domain', criterion.alpha_domain, global_step=epoch)
