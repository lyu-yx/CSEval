...
class SimpleDecoder(nn.Module):
    ...
    def forward(self, f1, f2, f3, f4, mask):
        ...
        #sem = self.softmax(logits)  # 输出 [batch, 6,352,352]，每个值是类别的概率

        # 2. 扩展mask的通道数，从 [batch, 1, 352, 352] 到 [batch, 6, 352, 352]，以便与output匹配
        mask = mask.expand(-1, 6, -1, -1)  # 扩展通道数
        ...
        target_logits = masked_sum / mask_area  # (batch, 6)

        # 对输出进行全局平均池化，将空间维度压缩
        #output = self.global_pool(output)  # 输出 [batch, 6, 1, 1]
        #output = self.softmax(output)  # 输出 [batch, 6,1,1]，每个值是类别的概率
        #output = output.view(output.size(0), -1)  # 展平为 [batch, 6]
