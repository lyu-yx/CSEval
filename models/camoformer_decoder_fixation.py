...
class Decoder(nn.Module):
    def __init__(self, channels):
        ...
        self.output_conv4 = nn.Conv2d(channels, 6, kernel_size=1)
        self.output_conv3 = nn.Conv2d(channels, 6, kernel_size=1)
        self.output_conv2 = nn.Conv2d(channels, 6, kernel_size=1)
        self.output_conv1 = nn.Conv2d(channels, 6, kernel_size=1)
        ...
        self.output_conv_fixation = nn.Sequential(
            ...
            # 最终输出层
            nn.Conv2d(channels // 2, 1, 1),  # 1x1卷积压缩到单通道
            nn.Sigmoid()  # 输出0-1
        )
    ...
    def classify_feature(self, feat, conv, mask_gt, fixation):
        x = conv(feat)
        x_softmax = self.softmax(x)  # 输出 [batch, 6,352,352]，每个值是类别的概率

        # 步骤 1: 用 argmax 得到每个像素的类别（预测类别）
        pred_label = x_softmax.argmax(dim=1)  # [B, H, W]，每个像素属于 [0~5] 的某个类
        mask = (pred_label != 0).float().unsqueeze(1)  # [B, 1, H, W]

        fixation = fixation.expand(-1, 6, -1, -1)  # 扩展通道数

        # 2. 扩展mask的通道数，从 [batch, 1, 88, 88] 到 [batch, 5, 88, 88]，以便与output匹配
        mask_gt = mask_gt.expand(-1, 6, -1, -1)  # 扩展通道数

        mask_area = mask * mask_gt * fixation
        masked_logits = x * mask_area  # [B, 6, H, W]
        masked_sum = masked_logits.sum(dim=[2, 3])  # [B, 6]，每类的 masked sum
        fixation_area = mask_area.sum(dim=[2, 3]).clamp(min=1e-6)  # (batch, 1)
    ...
    def forward(self, E4, E3, E2, E1, mask):
        ...
        fixation = self.output_conv_fixation(D1)
        fixation = F.interpolate(fixation, size=(mask.size(2), mask.size(3)), mode='bilinear', align_corners=True)
        ...
        out1,target_out1 = self.classify_feature(D1, self.output_conv1,mask,fixation)
