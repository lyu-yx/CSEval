...
from models.enhanced_components_dofixre import AdaptiveReceptiveFieldModule


class FeatureFusion(nn.Module):
    """Trainable feature fusion with normalization"""
    def __init__(self, channels):
        super().__init__()
        # Learn channel-wise weights for better flexibility
        self.alpha = nn.Parameter(torch.ones(channels, 1, 1))
    ...
    def forward(self, original_feature, adapted_feature):
        ...
        original_norm = self.norm1(original_feature)
        adapted_norm = self.norm2(adapted_feature)
        
        # Apply sigmoid to keep weights in [0,1] range and ensure stability
        alpha = torch.sigmoid(self.alpha)
        beta = torch.sigmoid(self.beta)
        
        return alpha * original_norm + beta * adapted_norm
