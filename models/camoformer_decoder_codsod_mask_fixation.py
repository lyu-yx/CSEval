...
class Decoder(nn.Module):
    def __init__(self, channels):
        ...
        self.output_conv4 = nn.Conv2d(channels, 10, kernel_size=1)
        self.output_conv3 = nn.Conv2d(channels, 10, kernel_size=1)
        self.output_conv2 = nn.Conv2d(channels, 10, kernel_size=1)
        self.output_conv1 = nn.Conv2d(channels, 10, kernel_size=1)
    ...
    def classify_feature(self, feat, conv, mask_gt):
        ...
        mask_gt = mask_gt.expand(-1, 10, -1, -1)  # 扩展通道数

        masked_logits = x * mask_gt
        masked_sum = masked_logits.sum(dim=[2, 3])
        fixation_area = mask_gt.sum(dim=[2, 3]).clamp(min=1e-6)  # (batch, 1)
        target_logits = masked_sum / fixation_area
    ...
    def forward(self, E4, E3, E2, E1, mask):
        ...
        fixation = fixation*mask


        # 仅处理 D1 层，其他尺度可删或保留调试
        out1 = self.output_conv1(D1)  # shape: [B, 10, H, W]

        mask_gt = mask.expand(-1, 10, -1, -1)  # 扩展通道数
        fixation_ = fixation.expand(-1, 10, -1, -1)  # 扩展通道数

        masked_logits = out1 * mask_gt * fixation_
        masked_sum = masked_logits.sum(dim=[2, 3])
        fixation_area = (mask_gt*fixation_).sum(dim=[2, 3]).clamp(min=1e-6)  # (batch, 1)
        target_logits = masked_sum / fixation_area


        return out1,target_logits,fixation
