...
from models.enhanced_decoder_dofixre3 import EnhancedDecoder
from models.enhanced_components_dofixre3 import DomainAdapter
...
class EnhancedDegreeNet(nn.Module):
    ...
    def __init__(self, channel=128):
        ...
        # Initialize with a small value to start with less adaptation
        self.adaptation_strength_4 = nn.Parameter(torch.tensor(0.1))
        self.adaptation_strength_3 = nn.Parameter(torch.tensor(0.1))
        self.adaptation_strength_2 = nn.Parameter(torch.tensor(0.1))
    ...
    def forward_enhanced(self, x, mask, training_mode=True):
        ...
        adapted_component_4, adapted_component_3, adapted_component_2, domain_logits = self.domain_adapter(fb4, fb3, fb2, training_mode)

        # Scale the adaptation for each level independently before adding
        fb4_adapted = fb4 + self.adaptation_strength_4 * adapted_component_4
        fb3_adapted = fb3 + self.adaptation_strength_3 * adapted_component_3
        fb2_adapted = fb2 + self.adaptation_strength_2 * adapted_component_2
        
        # Pass both original and adapted features to decoder
        pred, target_pred, fixation = self.decoder(
            fb4, fb3, fb2, fb1,                    # Original features for AdaptiveReceptiveField
            fb4_adapted, fb3_adapted, fb2_adapted,  # Adapted features for fusion
            mask
        )
