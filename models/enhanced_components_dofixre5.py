...
class DomainAdapter(nn.Module):
    ...
    def __init__(self, channels, adapter_dim=16):
        ...
        self.cod_adapter5 = nn.Sequential(
        ...
        self.sod_adapter5 = nn.Sequential(
            nn.Conv2d(channels, adapter_dim, 1),
            nn.BatchNorm2d(adapter_dim),
            nn.ReLU(inplace=True),
            nn.Conv2d(adapter_dim, channels, 1),
    ...
    def forward(self, features5, features4, features3, features2, training_mode=True):
        # Domain detection (only for training loss, not used in forward)
        domain_logits = self.domain_detector(features5) if training_mode else None

        # Apply both adapters and learn to weight them
        cod_adapted5 = self.cod_adapter5(features5)
        sod_adapted5 = self.sod_adapter5(features5)
        ...
        if domain_weights is not None:
            ...
            adapted_features5 = cod_weight * cod_adapted5 + sod_weight * sod_adapted5
        ...
        return adapted_features5,adapted_features4, adapted_features3, adapted_features2, domain_logits
